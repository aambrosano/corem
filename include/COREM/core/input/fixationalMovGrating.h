 ...
 * of object and background motion in the retina." Nature 423.6938 (2003):
 *401-408.
 *
...
class FixationalMovGrating : public Input {
protected:
  // Grating parameters
  int type1, type2;
  int tswitch;
  ...
  // Normal distributions for each interval (one for centre and one for
  // periphery)
  boost::random::normal_distribution<double> distribution1;
  boost::random::normal_distribution<double> distribution2;

  boost::random::linear_congruential_engine<unsigned long, 16807, 0, 2147483647>
      generator1;
  boost::random::linear_congruential_engine<unsigned long, 16807, 0, 2147483647>
      generator2;
  boost::random::linear_congruential_engine<unsigned long, 16807, 0, 2147483647>
...
public:
  ...
  // FixationalMovGrating(int X=25,int Y=25,double radius=1.0,double jitter =
  // 15.0,double period = 10.0,double step = 1.0,double luminance = 100.0,double
  // contrast = 0.5,double orientation = 0.0,double red_weight=1.0,double
  // green_weigh=1.0, double blue_weight=1.0,int t1 = 0,int t2 = 0,int ts = 0);
  FixationalMovGrating(unsigned int columns, unsigned int rows,
