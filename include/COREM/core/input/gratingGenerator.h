 ...
 * Cont  -> contrast of the grating. Between 0 and 1. If 1, then the image
 *reaches luminance level 0.
 * phi -> spatial phase of central pixel at time zero, in multiples of Pi, for a
 *COSINUS.
 * phi_t -> temporal phase of signal at time zero, in multiples of Pi, for a
 *COSINUS. (only if type=1.)
...
class EXPORT GratingGenerator : public Input {
protected:
  // Grating parameters
  int type;
  double step;
...
public:
  ...
  GratingGenerator(unsigned int columns, unsigned int rows,
                   std::map<std::string, param_val_t> params);
  // GratingGenerator(int ptype,double pstep,double plengthB,double
  // plength,double plength2,int pX,int pY,double pfreq,double pT,double
  // pLum,double pCont,double pphi,double pphi_t,double ptheta,double red,
  // double green, double blue,double pred_phi, double pgreen_phi,double
  // pblue_phi);
